...
#if defined(STM32F410XB)//STM32F410RB
#define _EE_SIZE (1024 * 64) //SKIP ALL THESE ADDRESS
//#define _EE_ADDR_INUSE (((uint32_t)0x08010000) | (_EE_SIZE*(_EE_USE_FLASH_PAGE_OR_SECTOR - 4)))
#define _EE_ADDR_INUSE ((uint32_t)0x08011000)
#define   _EE_PAGE_OR_SECTOR    SECTOR
#define _EE_FLASH_BANK FLASH_BANK_1
#define _EE_VOLTAGE_RANGE _EE_VOLTAGE
#if (_EE_USE_FLASH_PAGE_OR_SECTOR > 5)
...
#if defined(STM32F401XC)//STM32F401CCU6
...
#define _EE_ADDR_INUSE ((uint32_t)0x08020000)  //starting address from where you wanna write data(must be within the sector u r using )
#define   _EE_PAGE_OR_SECTOR    SECTOR
#define _EE_FLASH_BANK FLASH_BANK_1
#define _EE_VOLTAGE_RANGE _EE_VOLTAGE
#if (_EE_USE_FLASH_PAGE_OR_SECTOR > 5)
  ...
  HAL_ICACHE_Disable();
	...
	for (uint32_t i = 0; i < len; i += 2) {
		if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD,
				((i + startVirtualAddress)) + _EE_ADDR_INUSE,
				(uint64_t) (data[i] | (data[i + 1] << 8))) != HAL_OK) {
			HAL_FLASH_Lock();
#if _EE_ICACHE_CTRL == 1
		HAL_ICACHE_Enable();
#endif
	...
	for (uint32_t i = 0; i < len; i += 8) {
		uint8_t DoubleWord[8] = { data[i + 0], data[i + 1], data[i + 2], data[i
				+ 3], data[i + 4], data[i + 5], data[i + 6], data[i + 7] };
		if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD,
				((i + startVirtualAddress)) + _EE_ADDR_INUSE,
				(uint32_t) DoubleWord) != HAL_OK) {
			HAL_FLASH_Lock();
#if _EE_ICACHE_CTRL == 1
		HAL_ICACHE_Enable();
#endif
